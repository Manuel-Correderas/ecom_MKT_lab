# streamlit_app/pages/0b_📄_Términos_y_Privacidad.py
...
/* título */
.hdr{
  font-size:1.25rem; font-weight:900; color:#1f2e5e; margin-bottom:10px;
}

/* área scrolleable SIN fondo blanco ni borde */
.scroll{
  max-height: 60vh;             /* ajusta altura */
  overflow-y: auto;
...
/* botones */
.btn-azul{
  background:#0b3a91; color:#fff; border:none; border-radius:10px;
  padding:.7rem 1.2rem; font-weight:800; width:100%;
}
.btn-marron{
  background:#936037; color:#fff; border:none; border-radius:10px;
  padding:.7rem 1.2rem; font-weight:800; width:100%;
}
...
# ---------- estado ----------
accepted = st.session_state.get("accepted_terms", False)

c = st.columns([1,2,1])[1]
with c:
    st.markdown('<div class="panel">', unsafe_allow_html=True)
    st.markdown('<div class="hdr">📄 Términos de Uso y Privacidad – Ecom MKT Lab</div>', unsafe_allow_html=True)
...
""")
    ...
    st.write("")
    accepted = st.checkbox("He leído los términos de uso y privacidad", value=accepted, key="accepted_terms")

    st.write("")
    col1, col2 = st.columns(2)
    with col1:
        cont = st.button("CONTINUAR", disabled=not accepted)
    with col2:
        salir = st.button("SALIR")

    if cont and accepted:
        target = st.session_state.get("after_terms_page") or "pages/0c_📝_Alta_de_usuario.py"
